 ...
 * Provides improved functionality for sharing content across social media platforms
 * with better formatting, previews, and analytics
 */

import { generateId } from "@/lib/utils"
// Import the share analytics service
 ...
 */
export function trackShareEvent(platform: string, url: string, contentType = "unknown", contentId?: string): void {
  ...
  // Record the share event using the analytics service
  shareAnalyticsService.recordShareEvent(shareEvent).catch((err) => console.error("Failed to record share event:", err))
  ...
  if (typeof fetch !== "undefined") {
    ...
    }).catch((err) => console.error("Failed to log share event:", err))
  }
}
...
/**
 * Formats content for optimal social media sharing
 */
export function formatForSocialMedia(content: ShareableContent): ShareableContent {
  return {
    ...content,
    title: content.title.length > 60 ? content.title.substring(0, 57) + "..." : content.title,
    description: createShareableSummary(content.description, 200),
    hashtags: content.hashtags?.slice(0, 5) || [], // Limit hashtags to 5
  }
}

/**
 * Generates a preview image for sharing
 */
export async function generatePreviewImage(content: ShareableContent): Promise<string> {
  // In a real implementation, this would call an API to generate a preview image
  // For this example, we'll just return the existing image URL or a placeholder
  return content.imageUrl || `/placeholder.svg?height=630&width=1200&query=${encodeURIComponent(content.title)}`
}

// Export the enhanced social sharing service
