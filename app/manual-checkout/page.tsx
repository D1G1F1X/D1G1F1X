...
import type { OrderItem } from "./actions" // Import the type
...
import { buyPageDropdownProducts } from "@/lib/products" // Use the new filtered list
...
function ManualCheckoutFormContent() {
  ...
  const handleProductSelect = (itemId: string, selectedProductId: string) => {
    const product = buyPageDropdownProducts.find((p) => p.id === selectedProductId) // Use filtered list
    if (product) {
      setItems(
        items.map((item) =>
          item.id === itemId
        ...
        items.map((item) =>
          item.id === itemId
            ? {
                ...item,
                productId: undefined,
                name: item.isCustom ? item.name : "", // Keep custom name if already custom
                price: item.isCustom ? item.price : 0, // Keep custom price if already custom
                isCustom: true,
              }
            : item,
        ),
                ...
                <div>
                  ...
                  >
                    ...
                    <SelectContent className="bg-gray-700 text-white border-gray-600">
                      ...
                      {buyPageDropdownProducts.map(
                        (
                          product, // Use filtered list
                        ) => (
                          <SelectItem
                            key={product.id}
                            value={product.id}
